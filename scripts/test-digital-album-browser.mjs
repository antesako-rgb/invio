...
const virtual = {
  ...
  "editor-shell-double":
    'const R=require("react");module.exports={__esModule:true,default:(p)=>{R.useEffect(()=>{window.viewHeader=p.headerProps;window.viewMobile=p;});return R.createElement("div",null,p.sidebar,p.children);}};',
...
try {
  ...
  assert.equal(await page.$$eval(".sheet", (elements) => elements.length), 19);
  const initialOverflow = await page.evaluate(() => window.harness.overflow());
  assert.deepEqual(
    initialOverflow,
    [],
  ...
  for (const [image, ratio, disabled] of [
    ...
    assert.ok(frame.height <= 288 && frame.width <= 390, "Bounded crop frame");
  ...
  assert.equal(await page.evaluate(() => window.harness.editor.flush()), true);
  ...
  assert.equal(
    await page.evaluate(
      ({ x, y }) =>
        Boolean(
          document.elementFromPoint(x, y)?.closest("[data-album-photo-select]"),
        ),
  ...
  await page.mouse.up();
  await page.waitForFunction(() => window.harness.editor.activePageIndex === 4);
  ...
  for (const width of [769, 768]) {
    ...
    for (const count of [1, 2, 3, 5]) {
      ...
      for (let step = 0; step < count; step++) {
        ...
        const before = await page.evaluate(() => window.edge.visible.join(","));
  ...
  await page.waitForFunction(() => !!window.viewSidebar && !!window.viewHeader);
  ...
  for (const width of [1100, 390]) {
    ...
    for (const action of ["apply", "cancel", "close"]) {
      ...
      if (action === "apply") before.pages[5].photos[0].caption = "Crop page 6";
  ...
  await page.waitForFunction(() => window.viewMobile.mobileSnapPoint === 0.23);
  ...
  await page.evaluate(() => document.getElementById("native-history").remove());
  await page.evaluate(() => {
  ...
  await page.evaluate(() => {
    const text = [...document.querySelectorAll("[data-album-text-value]")].find(
  ...
  for (const tab of ["photos", "design"]) {
    ...
    assert.ok(
      await page.$eval("[data-base-ui-swipe-ignore]", (el) => el.scrollTop > 0),
      "Content must scroll without swiping Drawer",
    );
  }
  await page.evaluate(() => { window.drawerFixture.setTab("pages"); document.querySelector("[data-base-ui-swipe-ignore]").scrollTop=0; });
  await page.waitForSelector(
    'button[aria-label="' + messages.pages.move.replace("{number}", "1") + '"]',
  );
